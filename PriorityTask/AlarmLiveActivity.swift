...
struct AlarmLiveActivity: Widget {
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: AlarmAttributes<TaskMetadata>.self) { context in
            ...
            DynamicIsland {
                ...
                DynamicIslandExpandedRegion(.trailing) {
                    taskInfo(metadata: context.attributes.metadata)
                }
                ...
                // CHANGED: Passed priority instead of cookingMethod
                AlarmProgressView(priority: context.attributes.metadata?.priority,
    ...
    func lockScreenView(attributes: AlarmAttributes<TaskMetadata>, state: AlarmPresentationState) -> some View {
        VStack {
            HStack(alignment: .top) {
                alarmTitle(attributes: attributes, state: state)
                Spacer()
                taskInfo(metadata: attributes.metadata)
            }
    ...
    func bottomView(attributes: AlarmAttributes<TaskMetadata>, state: AlarmPresentationState) -> some View {
        HStack {
    ...
    @ViewBuilder func alarmTitle(attributes: AlarmAttributes<TaskMetadata>, state: AlarmPresentationState) -> some View {
    ...
    @ViewBuilder func taskInfo(metadata: TaskMetadata?) -> some View {
        // CHANGED: Checked for priority instead of method
        if let priority = metadata?.priority {
            HStack(spacing: 4) {
                // CHANGED: Used priority.rawValue and priority.indicatorImage()
                Text(priority.rawValue.localizedCapitalized)
...
struct AlarmProgressView: View {
    // CHANGED: Updated type to Priority? instead of TaskMetadata.Method?
        ...
        Group {
            ...
            case .countdown(let countdown):
                ProgressView(
                    ...
                    label: { EmptyView() },
                    currentValueLabel: {
                        // CHANGED: Used priority image
                        Image(systemName: priority?.indicatorImage() ?? "timer")
                            .scaleEffect(0.9)
                    })
...
struct AlarmControls: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            ...
            // CHANGED: Hardcoded .stopButton here because presentation.alert.stopButton
            // might be nil when using the new AlarmKit initializer, but we still want the button to show.
